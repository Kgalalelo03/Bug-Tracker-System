...
const updateIssue = ( id, summary, description, identifier, when, severity, assignedTo, targetDate) => {
  const issues = JSON.parse(localStorage.getItem('issues'));
  const currentIssue = issues.find(issue => issue.id == id);
  currentIssue.summary = prompt('Enter the updated summary:', summary);
  currentIssue.description = prompt('Enter the updated description:', description);
  currentIssue.identifier = prompt('Enter the updated identified by:', identifier);
  currentIssue.when = prompt('Enter the updated identified date:', when);
  currentIssue.severity = prompt('Enter the updated priority:', severity);
  currentIssue.assignedTo =prompt('Enter the updated assigned to:', assignedTo);
  currentIssue.targetDate  = prompt('Enter the updated target resolution date:', targetDate);
  localStorage.setItem('issues', JSON.stringify(issues));
  fetchIssues();
}
...
const fetchIssues = () => {
  ...
  for (var i = 0; i < issues.length; i++) {
    ...
    issuesList.innerHTML += `<div class="well">
                              ...
                              <p>Due: ${targetDate}<p>
                              <button onclick="updateIssue(${id})" class="btn btn-warning">Update</button>
